...
class MarketoApi
  ...
  def update_leads(action: "updateOnly", lookupField: "email", input: [])
    api_response do
      post("v1/leads.json", data: { action: action, lookupField: lookupField, input: input })
    end
  end

  def delete_leads(ids: [])
    api_response { post("v1/leads/delete.json", data: { input: ids.map { |id| { id: id } } }) }
  end

  private

  def access_token
    return @access_token if @access_token && @expires_at && @expires_at > Time.zone.now
    ...
    uri =
      URI.parse(
        "#{SiteSetting.marketo_identity}/oauth/token" + "?grant_type=client_credentials" +
  ...
  def api_response
    ...
    begin
      ...
      Rails.logger.warn(
        "[MarketoApi] API returned invalid JSON response: #{e.message}, body = #{response.body}",
      )
      return
    end
  ...
  def get(endpoint, params: {})
    ...
    uri =
      URI.parse(
        "#{SiteSetting.marketo_endpoint}/#{endpoint}?access_token=#{access_token}&#{query_string}",
      )
