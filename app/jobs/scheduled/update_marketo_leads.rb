  ...
  class UpdateMarketoLeads < ::Jobs::Scheduled
    ...
    def execute(args)
      ...
      if queue_size > SiteSetting.marketo_updates_size
        Rails.logger.warn("[UpdateMarketoLeads] Marketo user updates may be stuck in queue (current queue size = #{queue_size})")
      end
      ...
      input = User
        ...
        {
          email: email,
          SiteSetting.marketo_trust_level_field => trust_level,
        }
      end
      ...
      if !response
        user_ids.each do |user_id|
          Discourse.redis.lpush(DiscourseMarketo::UPDATES_QUEUE, user_id)
        end
      end
    end
  end
